...
function PerformInsert(pageID) {
    ...
    szSubFormReturnData.UniqeFieldValue = new Date().getTime();
    ...
    $("[FORM_INPUT='" + pageID + "']").each(function () {
        ...
        if (FormSet.applications[global_applicationid].forms[pageID].Sections[sid].Fields[fid].DisplaySubform == "yes") {
            ...
            szSubFormReturnData.UniqeFieldValue = UniqeFieldValue;
        ...
        if ($(this).attr("doProcess") == "false") { } else {
    ...
    if (ErrorLog != "") {
        ...
        if (child_item.PageID == pageID) {
            ...
            child_item.UniqueValue = szSubFormReturnData.UniqeFieldValue;
...
function UpdateSubTableForm(pageID, uid, child_item_) {
    if (uid == undefined || child_item_ == undefined)
        alert("undefined found");
    // Only store the info once - ?? [field,label]

    console.log(child_item_);
    var FormInfo = undefined;
    var bClean = true;
    var bOdd = false;
    ChildForms.forEach(function (r) {
        if (r.PageID == pageID && r.ParentFormUID == uid) {

            if (FormInfo == undefined) {
                FormInfo = new Array();
                for (var xSection = 0; xSection < FormSet.applications[global_applicationid].forms[pageID].Sections.length; xSection++) {
                    for (var xFld = 0; xFld < FormSet.applications[global_applicationid].forms[pageID].Sections[xSection].Fields.length; xFld++) {
                        if (FormSet.applications[global_applicationid].forms[pageID].Sections[xSection].Fields[xFld].DisplaySubform == "yes") {
                            var array = new Array();
                            array[0] = FormSet.applications[global_applicationid].forms[pageID].Sections[xSection].Fields[xFld].Label; // Header 
                            array[1] = FormSet.applications[global_applicationid].forms[pageID].Sections[xSection].Fields[xFld].DataField; // Field 
                            array[2] = "";  // Blank (index in sync layer
                            FormInfo.push(array);
                        }
                    }
                }

                for (var xForm = 0; xForm < FormInfo.length; xForm++) {
                    for (var xChildFieldIndex = 0; xChildFieldIndex < r.SyncArray.InsertLayerFields.length; xChildFieldIndex++) {
                        if (FormInfo[xForm][1].toString() == r.SyncArray.InsertLayerFields[xChildFieldIndex].Field) {
                            FormInfo[xForm][2] = xChildFieldIndex;
                            break;
                        }
                    }
                }
            ...
            // Run thru each field and find the subform info
            {
                if ($("[container='childResults'][pageid='" + child_item_.ParentForm + "'][fieldid='" + child_item_.ParentFieldID + "'][sectionid='" + child_item_.ParentSectionID + "'] tr").length == 0) {

                    var tr = $("<tr>");
                    for (var x = 0; x < FormInfo.length; x++) {
                        $(tr).append($("<th>").html(FormInfo[x][0]));
                    }
                    $(tr).append($("<th>").text("Actions"))
                    $("[container='childResults'][pageid='" + child_item_.ParentForm + "'][fieldid='" + child_item_.ParentFieldID + "'][sectionid='" + child_item_.ParentSectionID + "'] table thead")
                        .attr("class", "")
                        .append(tr)
            ...
            if (bClean) {
                $("[container='childResults'][pageid='" + child_item_.ParentForm + "'][fieldid='" + child_item_.ParentFieldID + "'][sectionid='" + child_item_.ParentSectionID + "'] table tbody").html("");
                bClean = false;
            }
            tr = $("<tr>").addClass((bOdd ? "odd" : "even"));

            {
                bOdd = !bOdd;
                for (var x = 0; x < FormInfo.length; x++) {
                    $(tr).append($("<td>").html((FormInfo[x][2].toString() == "" ? "NA" : r.SyncArray.InsertLayerFields[FormInfo[x][2]].Value)))
                }
                /// DISABLE SUBFORM EDIT (for now)
                $(tr).append($("<td>")
                    .append(
                        $("<a>")
                            .attr("uid", r.UniqueValue)
                            .text("Edit")
                            .on("click", function () {
                                EditRecord(this);
                            })
                ...
                $("[container='childResults'][pageid='" + child_item_.ParentForm + "'][fieldid='" + child_item_.ParentFieldID + "'][sectionid='" + child_item_.ParentSectionID + "'] table tbody")
                    .append(tr)
                /*   $("[container='childResults'][pageid='" + child_item_.ParentForm + "'][fieldid='" + child_item_.ParentFieldID + "'][sectionid='" + child_item_.ParentSectionID + "']").append($("<p>").html(
...
function PerformOnlineSyncOperation(immidiatesync, pageID) {
    ...
    $.ajax({
        ...
        data: JSON.stringify({
            Input: immidiatesync,
            _UserInfo: loggedinuser,
            _AppInfo: ApplicationInformation,
...
function EditRecord(control) {
    $("[subformBack]").hide();
    var uid = $(control).attr("uid");
    var parentElement = $(control).closest("*[pageid]");
    
    var fieldID = $(parentElement).attr("fieldid");
    var sectionID = $(parentElement).attr("sectionid");
    var pageID = $(parentElement).attr("pageid");
    ...
    var parentForm = new Date().getTime();
    ChildForms.forEach(function (e) { if (e.UniqueValue == uid) { parentForm = e.PageUID  } })
